...
RETURNS interval AS $$
BEGIN
  RETURN (((random_boundedgauss(-6.0, 6.0) * 100.0) + 600.0) * 6000.0)::int * interval '1 ms';
END;
$$ LANGUAGE plpgsql STRICT;
...
-------------------------------------------------------------------------------
-- Functions for home/work regions and nodes
-------------------------------------------------------------------------------

-- Create home/work regions and nodes

DROP FUNCTION IF EXISTS berlinmod_createRegionsNodes;
CREATE OR REPLACE FUNCTION berlinmod_createRegionsNodes()
RETURNS void AS $$
BEGIN
  ...
  SELECT id, id, population, PercPop,
    SUM(PercPop) OVER (ORDER BY id ASC ROWS BETWEEN UNBOUNDED PRECEDING 
      AND CURRENT ROW) AS CumProb, geom
  FROM Municipalites;
  ...
  SELECT id, id, NoEnterp, PercEnterp,
    SUM(PercEnterp) OVER (ORDER BY id ASC ROWS BETWEEN UNBOUNDED PRECEDING
      AND CURRENT ROW) AS CumProb, geom
  FROM Municipalites;
...
-- Create home/work regions and nodes
  PERFORM berlinmod_createRegionsNodes();
